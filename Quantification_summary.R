...
Produce_combined_quant_without_LFQ <- function(path, dataset){
  #dataset='HYC_GUI'
  pathDIA_BERT=paste0(path, '/', dataset)
  prt1='_quant_sum6_precursor_area_diann_combine.csv'
  pgt1='_quant_sum6_protein_area_diann_combine.csv'
  #precursors
  DIA_BERT_prs <- dir(pathDIA_BERT, paste0('.*',prt1))
  if(length(DIA_BERT_prs)==0){
    prt1='.mzML_precursor.csv'
    pgt1='.mzML_protein.csv'
    DIA_BERT_prs = dir(pathDIA_BERT, paste0('.*',prt1))
  }
    
  DIA_BERT_precursor <- data.frame()
  for(i in DIA_BERT_prs){
    T1 = read.csv(paste0(pathDIA_BERT,'/',i))
    ...
    DIA_BERT_precursor = rbind(DIA_BERT_precursor,T1)
  }
  DIA_BERT_precursor$file_name=gsub('.mzML','',DIA_BERT_precursor$file_name)

  #Proteins
  DIA_BERT_pgs <- dir(pathDIA_BERT, paste0('.*',pgt1))
  DIA_BERT_protein <- data.frame()
  for(i in DIA_BERT_pgs){
    #i=DIA_BERT_pgs[1]
    T1 = read.csv(paste0(pathDIA_BERT,'/',i))
    colnames(T1)[which(colnames(T1)=='transition_group_id')]='Precursor'
    colnames(T1)[which(colnames(T1)=='ProteinID')]='Protein_id'
    ...
    T1$file_name = fn
    DIA_BERT_protein = rbind(DIA_BERT_protein,T1)
  }
  DIA_BERT_protein$file_name=gsub(pgt1,'',DIA_BERT_protein$file_name)

  ltw_precursor <- function(data){
    #data=DIA_BERT_precursor
  ...
  DIA_BERT_precursor1=ltw_precursor(DIA_BERT_precursor)
  DIA_BERT_protein1  =ltw_Protein(DIA_BERT_protein)
  write.table(cbind(Precursor=rownames(DIA_BERT_precursor1),DIA_BERT_precursor1),paste0(pathDIA_BERT,'/DIA_BERT_precursor1_',dataset,'.tsv'), sep='\t',row.names = F)
  write.table(DIA_BERT_protein1,paste0(pathDIA_BERT,'/DIA_BERT_protein1_',dataset,'.tsv'), sep='\t',row.names = F)
}
###HYC set
...
###DIA-NN_lib res
path='D:/LZW/Guomics/DIA-BERT/Benchmark/fdrs_DIANN19/DIA_NN_19_lib'
setwd(path)
