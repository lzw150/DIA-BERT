...
peptides_per_protein <- function(data,Dataset){
  ...
  rownames(D1)=rn
  for(i in cn){
    T1=data[data$Filename==i, ]
    T2=table(T1$Protein_id[T1$Protein_id!='' & T1$Dataset==Dataset])
    D1[names(T2),i]=T2
  }
...
comparison_of_quantification <- function(dataset,filenames,prs_file,pgs_file,rename,type,proteome1,proteome2,pdfwidth=8,subdir='MS_BERT/',softaware='In MS-BERT '){
  ...
  prs_quant_1 <- prs_quant_[,-c(1:3)]
  rownames(prs_quant_1) <- Precursor_
  colnames(prs_quant_1) <- gsub('.mzML$','', colnames(prs_quant_1))
  ...
  colnames(pgs_quant_) <- gsub('.mzML$','', colnames(pgs_quant_))
